"""
Stage 3 for Yates: classification.json 을 따라 problems_png/<id>.png 를
...
def main():
    ...
    if missing_png:
        print(f"\nMissing source PNGs: {missing_png[:10]}{'...' if len(missing_png)>10 else ''}")
    print(f"\nTotal distributed: {sum(counts.values())}")
